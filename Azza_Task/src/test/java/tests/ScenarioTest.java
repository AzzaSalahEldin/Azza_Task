...
public class ScenarioTest {
	...
	public Object[][] testDataExample() throws InvalidFormatException, IOException {
		...
		return configuration.readFromExcelBySheet(filePath, "Sheet1");
	}

	@BeforeMethod
	public void scenarioTest_BeforeMethod() {
		setup = new setUpEnv();
	...
	@AfterMethod
	public void scenarioTest_AfterMethod(ITestResult TestCaseResult) throws IOException {
		if(TestCaseResult.getStatus()==ITestResult.FAILURE) {
			screen_shot.table_screenshot(driver,".\\ScreenShotsfiles\\"+TestCaseResult.getName()+".png");
			base.closeDriver();
		}
		if(TestCaseResult.getStatus()==ITestResult.SUCCESS) {
			screen_shot.table_screenshot(driver,".\\ScreenShotsfiles\\"+TestCaseResult.getName()+".png");
			base.closeDriver();
		}	
	}
